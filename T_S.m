...
for index = 1:10
    for i = 1:length(S)
        S_ref = S(i);
        [ t, w ,wf] = T0_iteration(0.25,W0_guess,S_ref,index);
        T0(i,index) = t;
        W0(i,index) = w;
        Wf(i,index) = wf;
    end
...
figure(1)
hold on
plot(S,T0(:,1),S,T0(:,2),S,T0(:,3),S,T0(:,4),S,T0(:,5),S,T0(:,6),S,T0(:,7),S,T0(:,8),S,T0(:,9),S,T0(:,10),S,1.0258e6*ones(length(S),1),'LineWidth',2)
xlabel('S_{ref} [m^2]')
...
for i = 1:length(S)
    for j = 1: length(T_COC)
        ...
        while (abs(W0 - W0_old) > tol)    
            W0_old = W0;
            %call Tail_Sizing
            [S_ht,S_vt] = Tail_Sizing(S(i),L_fus,D_fus);
            %call Drag Polar
            [CD0, K, CL_max] = Drag_Polar(c_f, Swet_Sref, A, e);
            %call Empty_Weight
            [We] = Empty_WeightIII(W0,T_COC(j),S(i),L_fus,D_fus,S_ht,S_vt,c_f,Swet_Sref,A,e);
            %call Fuel Weight
